"""Geodesic model — proper size — 32B teacher — WikiText-103.

Student: H=2048, intermediate=8192, ~70M trainable
With 10 ODE steps = 500M effective MLP compute (above manifold floor)
Teacher: 32B (64 layers, 5120 hidden — best quality supervision)
Data: WikiText-103 (135M tokens)
"""
import torch
...
print("=" * 70)
print("GEODESIC v2 — proper bulk, 32B teacher, real data")
print("=" * 70)
...
# Student dimensions — 0.6B total (matches proven manifold size)
H_S = 2048
INTER_S = 44000
N_HEADS_S = 16
HEAD_DIM_S = H_S // N_HEADS_S  # 128

print(f"Student: H={H_S}, inter={INTER_S}, heads={N_HEADS_S}, hd={HEAD_DIM_S}")
...
# ═══════════════════════════════════════════════════════
# Student model — properly sized
# ═══════════════════════════════════════════════════════

class ContextAttention(nn.Module):
    def __init__(self):
        super().__init__()
        q_dim = N_HEADS_S * HEAD_DIM_S
        self.q_proj = nn.Linear(H_S, q_dim, bias=False)
        self.k_proj = nn.Linear(H_S, HEAD_DIM_S, bias=False)  # 1 KV head
        self.v_proj = nn.Linear(H_S, HEAD_DIM_S, bias=False)
    ...
    def forward(self, h, k_cache=None, v_cache=None):
        ...
        k = self.k_proj(h_norm).view(B, T, 1, HEAD_DIM_S).transpose(1, 2)
        v = self.v_proj(h_norm).view(B, T, 1, HEAD_DIM_S).transpose(1, 2)
        if k_cache is not None:
            k = torch.cat([k_cache, k], dim=2)
            v = torch.cat([v_cache, v], dim=2)
        k_exp = k.expand(B, N_HEADS_S, -1, HEAD_DIM_S)
        v_exp = v.expand(B, N_HEADS_S, -1, HEAD_DIM_S)
...
class GeodesicProper(nn.Module):
    def __init__(self):
        super().__init__()
        # Own embedding and lm_head at student hidden dim
        self.embed = nn.Embedding(VOCAB, H_S)
        self.lm_head = nn.Linear(H_S, VOCAB, bias=False)
        # Share weights (tied embeddings)
...
student = GeodesicProper().to(device).float()
...
print(f"\nTraining {N_STEPS} steps, batch={BATCH}, 32B teacher")
...
for step in range(N_STEPS):
    ...
    with torch.no_grad():
        t_logits = teacher(batch).logits  # [B, T, 151936]

    s_logits = student.forward_sequence(batch)  # [B, T, 151936]
    ...
    if step % 200 == 0 or step == N_STEPS - 1:
        ...
        print(f"{step:>7} {avg:>8.4f} {lr_now:>10.6f} {vram:>5.1f}G {spd:>6.1f}s {eta:>5.1f}h",
              flush=True)
    ...
    if (step + 1) % 1500 == 0:
        ...
        }, os.path.join(SAVE_DIR, f"geodesic_proper_step{step+1}.pt"))
