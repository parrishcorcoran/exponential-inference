"""Early exit v2: monkey-patch layer forwards to actually skip computation."""
import torch, torch.nn.functional as F, time, json
device = 'cuda'

print("="*70, flush=True)
print("EARLY EXIT v2 — Qwen3-14B (monkey-patched skip)", flush=True)
print("="*70, flush=True)
...
# Monkey-patch: wrap each layer to skip when exit_state says so
exit_state = {"layer": None}
...
for i in range(N_LAYERS):
    ...
    def make_patched(layer_idx, orig_fwd):
        def patched_forward(hidden_states, **kwargs):
            # If we already exited at an earlier layer, pass through unchanged
            if exit_state["layer"] is not None and layer_idx > exit_state["layer"]:
                # Return hidden_states unchanged, skip all computation
                return (hidden_states,)
            return orig_fwd(hidden_states, **kwargs)
        return patched_forward

    layer.forward = make_patched(i, original_forwards[i])

# Add exit checks at specific layers via a hook on the model's forward
CHECK_LAYERS = set(range(15, N_LAYERS, 5))  # 15, 20, 25, 30, 35
...
for cl in CHECK_LAYERS:
    def make_check(layer_idx):
        ...
        return hook
    check_hooks.append(model.model.layers[cl].register_forward_hook(make_check(cl)))

# Sweep tau
print("\n--- Early Exit Sweep ---", flush=True)
for TAU in [0.99, 0.95, 0.90, 0.80, 0.70]:
    current_tau = TAU
    ...
    # Speed
    with torch.no_grad(): model(test_ids)  # warmup
    torch.cuda.synchronize(); t0 = time.time()
...
# Cleanup
for h in check_hooks: h.remove()
for i in range(N_LAYERS):
    model.model.layers[i].forward = original_forwards[i]

print(f"\n  Baseline: {baseline_tps:.1f} tok/s", flush=True)

results = {"model": "Qwen3-14B", "baseline_tps": baseline_tps}
with open("machines/strix_halo/results/early_exit_14b_v2.json", "w") as f:
    json.dump(results, f, indent=2)
print(f"Saved early_exit_14b_v2.json", flush=True)
