"""Stage 222 — 0.6B with 218's protocol + FIXED λ_bimodal in phase 2.

Stage 218's protocol got -3.85 nats (best). Phase 2 added -1.07 of that
DESPITE its λ_bimodal regularizer never engaging (PID couldn't ramp λ
above 0 because drift exceeded band when body unfroze).

This stage is 218 with that bug fixed: λ_bimodal = 1e-2 FIXED in phase 2.
Hypothesis: with regularizer actually engaged, phase 2's body movement
gets DIRECT grid gradient (not just CE-driven drift), so K=1 reduction
in phase 2 should compound much more than -1.07.

Best case projection:
  Phase 1: -2.78 nats (validated, lever-only laser zone)
  Phase 2 with fixed regularizer: maybe -3 nats (vs broken's -1.07)
  Total: -5 to -6 nats — through 218's wall.

Variance penalty dropped (saves memory, redundant per math).

Protocol:
  Phase 1 (2000 steps):
    Body W_fp + β_g  FROZEN
    Compensation levers TRAINING
    γ slow-PID ramps 0 → 0.95
    Loss = CE only (no regularizer; body frozen so it'd do nothing)

  Phase 2 (2000 steps):
    Body W_fp + β_g  UNFROZEN
    Compensation levers STILL TRAINING
    γ HELD at phase-1-end value
    Loss = CE + λ_bimodal·||W − sign(W)·α||²    (FIXED λ=1e-2, ALWAYS ON)
    → body gets CE gradient at γ=0.33 forward
    → body gets bimodal gradient pulling to ±α grid
    → β_g gets gradient via γ·β_g forward path
    → levers continuously absorb activation pressure

K=1 diagnostic every 250 steps to track whether phase 2 keeps making
progress or saturates.
"""
import json
import sys
import time
...
LAMBDA_BIMODAL = 1e-2   # FIXED, ACTIVE in phase 2 (the fix)
...
RESULTS_PATH = Path("results/stage222_218_with_fixed_reg.json")
...
print("\nBuilding architecture (218 protocol)...", flush=True)
...
def train_step(batch, λ_bimodal=0.0):
...
# ─── PHASE 1: 218's lever-only laser zone discovery ───
print(f"\n{'─'*60}")
...
for step in range(1, PHASE_1_STEPS + 1):
    ...
    ce_loss = train_step(batch, λ_bimodal=0.0)
...
# ─── PHASE 2: body+β_g UNFROZEN, FIXED λ_bimodal ACTIVE ───
print(f"\n{'─'*60}")
print(f"PHASE 2 — body+β_g UNFROZEN, levers continue, γ held at {phase1_end_gamma:.3f}")
print(f"          λ_bimodal={LAMBDA_BIMODAL} FIXED (the fix vs Stage 218)")
print('─'*60, flush=True)
...
for step in range(1, PHASE_2_STEPS + 1):
    batch = sample_batch(train_tokens, BATCH_SIZE, SEQ_LEN, rng)
    ce_loss = train_step(batch, λ_bimodal=LAMBDA_BIMODAL)
...
print(f"\n{'─'*60}")
print("STAGE 222 RESULT (218 protocol + fixed regularizer in phase 2):")
print('─'*60)
print(f"  T0:                {T0:.4f}")
...
print(f"\n  vs 218 reference: -3.85 nats")
print(f"  vs hypothesis:    expected -5 to -6 nats with regularizer engaged")
