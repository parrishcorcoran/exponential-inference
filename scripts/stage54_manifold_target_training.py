"""
Stage 54 — Test theory #6: train against measured manifold, not teacher output.

Core claim: for any (context, next_token) pair, the next token has a
measurable manifold coordinate — independent of any teacher. Train a
small model to hit that coordinate. The teacher's output is not in the
loss.

Scoped pipeline test on Mac (not the full "break 400B" experiment):
  - Use Qwen3-0.6B's input embedding as the tokenizer's manifold instrument
    (ONE piece of ONE teacher, not the whole model — we never look at
    teacher hidden states or logits during training)
  - PCA project embeddings to rank k (try k ∈ {10, 64, 512})
  - Target per training example: projected-embedding of next_token
  - Student: 1 attention + 1 SwiGLU MLP (~3-5M trainable params)
  - Loss: cosine distance between predicted vector and target
  - Eval: next-token top-1 accuracy on held-out corpus
  - Compare student's next-token prediction to Qwen3-0.6B teacher's
    on the same held-out

If the student's held-out accuracy approaches or exceeds the teacher's
(at any rank), the "manifold as training target" pipeline works. If it
collapses to near-random, the scoped test fails and we need richer
measurement.

Note: this can't *prove* the teacher-ceiling-break claim on Mac
(requires multi-teacher ensemble, larger scale). But it can confirm the
...
def train_student(student, pairs, E_proj, embed_basis_mean, P, device, steps, batch_size, lr):
    """Train student to predict PCA-projected embedding of next token via cosine loss."""
    ...
    for step in range(steps):
        ...
        # Target: projected embedding of next token
        with torch.no_grad():
            tgt_emb = student.embed(target_ids).float()          # [B, H]
            tgt_proj = (tgt_emb - embed_basis_mean) @ P           # [B, k]

        # Student forward
        h_out = student(ctxs)                                     # [B, T, H]
        pred_last = h_out[:, -1, :].float()                       # [B, H]
        # Project through the same basis
        pred_proj = (pred_last - embed_basis_mean) @ P            # [B, k]

        # Cosine loss (maximize cosine → minimize 1 - cos)
        cos = F.cosine_similarity(pred_proj, tgt_proj, dim=-1)
        loss = (1 - cos).mean()
        ...
        if step % max(1, steps // 10) == 0 or step == steps - 1:
            print(f"    step {step:>4d}/{steps}  loss={loss.item():.4f}", flush=True)
...
def main():
    ...
    for k in ranks:
        ...
        loss_hist = train_student(
            student, train_pairs, E_full, mu, P, device,
