"""Stage 223 — Push + Pull regularizers + best-state checkpointing.

Stage 222 kept hitting peaks then losing them (e.g., P2 step 750 hit +7.78
new best, by step 1000 had regressed to +8.57). Two structural fixes:

1. PULL regularizer (Mexican-hat / double-well potential):
   L_pull = λ_pull · ((W/α)² − 1)²
   Minima at W = ±α (the grid points)
   Gradient pulls weights toward nearest grid + ANCHORS at grid
   (push has zero gradient at grid; pull has zero gradient too,
    but it's a STABLE minimum, not a unstable point — body sticks)

2. BEST-STATE CHECKPOINTING:
   Save full param snapshot at every new K=1 minimum
   Restore at end of run so reported state matches best K=1 measured

Combined loss in phase 2:
  L = CE + λ_push·||W−sign(W)·α||² + λ_pull·((W/α)²−1)²

  λ_push = 1e-2   (linear push: strong far from grid, weakens near)
  λ_pull = 1e-3   (Mexican hat: weak far, ANCHORS near grid)

Protocol (same shape as 222):
  Phase 1 (2000 steps): body+β_g frozen, levers train, γ ramps
  Phase 2 (2000 steps): body+β_g unfrozen, levers continue, regularizers ACTIVE

  K=1 diagnostic every 250 steps + state snapshot if new best.
"""
import json
import sys
import time
...
LAMBDA_PUSH = 1e-2   # bimodal squeeze (linear push toward grid)
LAMBDA_PULL = 1e-3   # Mexican-hat (anchors at grid)
...
RESULTS_PATH = Path("results/stage223_pull_checkpoint.json")
...
# ─── PUSH regularizer (linear bimodal squeeze) ───
def push_loss(model):
    total = 0.0; n = 0
    for mod in model.modules():
...
# ─── PULL regularizer (Mexican-hat double-well) ───
def pull_loss(model):
    """L_pull = ((W/α)² − 1)² per group, mean.
    Minima at W = ±α (anchors at grid).
    """
    total = 0.0; n = 0
    for mod in model.modules():
        if isinstance(mod, AdiabaticQuantizedLinear) and mod.has_groups:
            W = mod.weight_fp.float()
            Wg = W.reshape(mod.out_features, mod.in_features // mod.group_size, mod.group_size)
            alpha = mod.alpha.float()
            normalized_sq = (Wg / alpha.clamp(min=1e-8)) ** 2
            total = total + ((normalized_sq - 1.0) ** 2).mean()
            n += 1
    return total / max(n, 1)
...
def snapshot_state_to_cpu(model):
    """Snapshot all parameters AND adiabatic buffers to CPU."""
    snap = {}
    for n, p in model.named_parameters():
        snap[n] = p.detach().cpu().clone()
    # Also snapshot γ buffer state (architectural state)
    snap["__gamma__"] = get_gamma(model)
    return snap


def restore_state_from_cpu(model, snap):
    """Restore parameters and γ from snapshot."""
    with torch.no_grad():
        for n, p in model.named_parameters():
            if n in snap:
                p.data.copy_(snap[n].to(p.device))
        if "__gamma__" in snap:
            set_gamma(model, snap["__gamma__"])
...
print("\nBuilding architecture (push+pull)...", flush=True)
...
print(f"\nBody: {n_body:,}  β_g: {n_beta:,}  Levers: {n_lever:,}", flush=True)
...
def train_step(batch, λ_push=0.0, λ_pull=0.0):
    ...
    total = ce_loss
    if λ_push > 0:
        total = total + λ_push * push_loss(model)
    if λ_pull > 0:
        total = total + λ_pull * pull_loss(model)
    optimizer.zero_grad()
    total.backward()
    optimizer.step()
...
def maybe_save_best(model, k1_now, step, phase):
    """If k1_now is new best, snapshot the model state to CPU."""
    global best_k1, best_state, best_step
    if k1_now < best_k1:
        best_k1 = k1_now
        best_state = snapshot_state_to_cpu(model)
        best_step = step
        return True
    return False
...
# ─── PHASE 1 ───
print(f"\n{'─'*60}")
print(f"PHASE 1 — find laser zone (body+β_g FROZEN, levers train)")
print('─'*60, flush=True)
...
for step in range(1, PHASE_1_STEPS + 1):
    ...
    ce_loss = train_step(batch, λ_push=0.0, λ_pull=0.0)
    ...
    if step % K1_DIAG_EVERY == 0:
        ...
        improved = maybe_save_best(model, k1_now, step, "P1")
        marker = " ⭐ SAVED" if improved else ""
        print(f"  ── P1 step {step:>4} K=1 DIAG: drift={k1_now:+.4f}  "
              f"(best: {best_k1:+.4f}){marker}", flush=True)
        k1_trajectory.append({"step": step, "phase": "P1",
                              "k1_drift": float(k1_now), "gamma": float(current_gamma)})
        model.train()

# ─── PHASE 1 → 2 transition: restore best-K=1 state ───
print(f"\n  P1 END. Best K=1 in P1: {best_k1:+.4f} at step {best_step}")
if best_state is not None:
    print(f"  Restoring P1 best-K=1 state for phase 2 start...", flush=True)
    restore_state_from_cpu(model, best_state)
    # Verify
    k1_check = k1_drift_now(model, val_tokens, T0)
    print(f"  After restore: K=1 drift = {k1_check:+.4f} (should be ≈ {best_k1:+.4f})",
          flush=True)
phase1_end_gamma = get_gamma(model)


# ─── PHASE 2: body+β_g UNFROZEN, push + pull regularizers ACTIVE ───
print(f"\n{'─'*60}")
print(f"PHASE 2 — body+β_g UNFROZEN, levers continue")
print(f"          γ held at {phase1_end_gamma:.3f}")
print(f"          λ_push={LAMBDA_PUSH}  λ_pull={LAMBDA_PULL}  (BOTH ACTIVE)")
print('─'*60, flush=True)
...
for step in range(1, PHASE_2_STEPS + 1):
    batch = sample_batch(train_tokens, BATCH_SIZE, SEQ_LEN, rng)
    ce_loss = train_step(batch, λ_push=LAMBDA_PUSH, λ_pull=LAMBDA_PULL)
    ...
    if step % K1_DIAG_EVERY == 0:
        k1_now = k1_drift_now(model, val_tokens, T0)
        improved = maybe_save_best(model, k1_now, step, "P2")
        marker = " ⭐ SAVED" if improved else ""
...
# ─── Final restore best state ───
print(f"\n  Run complete. Best K=1: {best_k1:+.4f} at step {best_step}")
if best_state is not None:
    print(f"  Restoring best state for final report...", flush=True)
    restore_state_from_cpu(model, best_state)
    final_check = k1_drift_now(model, val_tokens, T0)
    print(f"  Final after restore: K=1 = {final_check:+.4f}", flush=True)
    final_k1 = final_check
else:
    final_k1 = k1_drift_now(model, val_tokens, T0)


print(f"\n{'─'*60}")
print("STAGE 223 RESULT (push + pull + best-state checkpointing):")
print('─'*60)
print(f"  T0:                {T0:.4f}")
print(f"  K=1 initial:       {k1_initial:+.4f}")
print(f"  K=1 final (best):  {final_k1:+.4f}")
print(f"  Reduction:         {k1_initial - final_k1:+.4f} nats "
      f"({100*(1 - final_k1/max(k1_initial, 1e-6)):.1f}%)")
print(f"  Best step:         {best_step}")
print(f"\n  vs prior runs:")
print(f"    218: -3.85 nats")
print(f"    222: best -3.78 (lost by end)")
print(f"    223: {-(k1_initial - final_k1):+.2f} nats")
